...
void draw() {
  ...
  if (state == STATE_START) {
    ...
    if (state == STATE_PLAY) {
      if (yourturn) {
        text("YOUR TURN", 2, 2, width - 4, 14);
      } 
      else {
        text("Waiting...", 2, 2, width - 4, 14);
      }
    } 
    else {
      if (result == RESULT_DRAW) {
        text("Draw!", 2, 2, width - 4, 14);
      } 
      else if (result == yourpiece) {
        text("You WIN!", 2, 2, width - 4, 14);
      } 
      else {
        text("You LOSE...", 2, 2, width - 4, 14);
      }
    }
    translate(0, 14);
    stroke(0);
...
void libraryEvent(Object library, int event, Object data) {
  if (library == bt) {
    ...
    case MBluetooth.EVENT_DISCOVER_DEVICE:
      msg = "Found device at: " + ((MDevice) data).address + "...";
      break;
    case MBluetooth.EVENT_DISCOVER_DEVICE_COMPLETED:
      msg = "Found " + ((MDevice[]) data).length + " devices, looking for game...";
      break;
    case MBluetooth.EVENT_DISCOVER_SERVICE:      
      msg = "Found game on " + ((MService[]) data)[0].device.address + "...";
      break;
    case MBluetooth.EVENT_DISCOVER_SERVICE_COMPLETED:
      services = (MService[]) data;
      msg = "Search complete.";
      break;
    case MBluetooth.EVENT_CLIENT_CONNECTED:
      c = (MClient) data;
      yourturn = true;
      yourpiece = PIECE_X;
      state = STATE_PLAY;

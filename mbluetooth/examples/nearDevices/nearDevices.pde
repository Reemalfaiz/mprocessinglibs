 ...
 *
 * $Id: nearDevices.pde 147 2006-07-31 02:00:44Z marlonj $
 */
...
void draw()
{
  ...
  int angleFP = itofp(TWO_PI / numDevices);
  int maxDistanceFP = itofp(maxDistance);

  for(int i=0; i<numDevices; i++)
  {
    int x = fptoi(mul(sin(mul(angleFP,i)),maxDistanceFP));
    int y = fptoi(mul(cos(mul(angleFP,i)),maxDistanceFP));
